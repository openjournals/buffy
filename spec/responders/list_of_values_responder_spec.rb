...
describe ListOfValuesResponder do
  ...
  describe "#process_message" do
    ...
    describe "labeling" do
      it "should process labeling when adding the first value" do
        msg = "@botsci add @xuanxu to reviewers"
        @responder.match_data = @responder.event_regex.match(msg)
      ...
      it "should process reverse labeling when removing the last value" do
        msg = "@botsci remove @xuanxu from reviewers"
        @responder.match_data = @responder.event_regex.match(msg)
  ...
  describe "documentation" do
    ...
    it "#description should include name" do
      expect(@responder.description[0]).to eq("Add to this issue's observers list")
      expect(@responder.description[1]).to eq("Remove from this issue's observers list")
    end

    it "#example_invocation should use custom sample value if present" do
      expect(@responder.example_invocation[0]).to eq("@botsci add @observer_username to observers")
      expect(@responder.example_invocation[1]).to eq("@botsci remove @observer_username from observers")
    end

    it "#example_invocation should have default sample value" do
      @responder.params = { name: "observers" }
      expect(@responder.example_invocation[0]).to eq("@botsci add xxxxx to observers")
      expect(@responder.example_invocation[1]).to eq("@botsci remove xxxxx from observers")
    end
  end
